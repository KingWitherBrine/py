def meet_condition(patient_zero, k, matrix, cows_state, N):  # O(T)
  infected = [0] * (N + 1)  # 1 extra to make it convenient for 1-based index
  ...
  for i, cow in enumerate(infected[1:]):
    if str(cow) != cows_state[i]:
      all_true = False
      break
  # return "".join(map(str, (infected[1:]))) == cows_state
  return all_true
...
def solve(matrix, cows_state, N, T):
  ...
  lower_k, upper_k = T, 0
  for i in range(1, N + 1):  # Check all possiblities: O(N*T^2)
    for k in range(T + 1):
